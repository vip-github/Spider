...
public class MongoBbsSpout extends BaseRichSpout {
	...
	public void nextTuple() {
		try {
			...
			if (null == index) {
				index = new Index(Bbs.class.getSimpleName().toLowerCase(), 0);
			}
			Query<Bbs> query = GlobalObject.crawlerMongo.getDatastore().find(Bbs.class);
			Iterator<Bbs> iterator = query.field(FieldsConstant.timestamp).greaterThan(index.getTimestamp()).iterator();
			while (iterator.hasNext()) {
				Bbs bbs = iterator.next();
				collector.emit(new Values(bbs));
			}
			collector.emit(new Values(Config.batch));
			Utils.sleep(60000);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("取出论坛数据异常：" + e.toString());
		}
	}
