...
public class MongoNewsSpout extends BaseRichSpout {
	...
	public void nextTuple() {
		try {
			...
			if (null == index) {
				index = new Index(News.class.getSimpleName().toLowerCase(), 0);
			}
			Query<News> query = GlobalObject.crawlerMongo.getDatastore().find(News.class);
			Iterator<News> iterator = query.field(FieldsConstant.timestamp).greaterThan(index.getTimestamp()).iterator();
			...
			e.printStackTrace();
			logger.error("取出新闻数据异常：" + e.toString());
		}
	}
