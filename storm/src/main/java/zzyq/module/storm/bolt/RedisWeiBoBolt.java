...
public class RedisWeiBoBolt extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		String data = "";
		try {
			data = tuple.getStringByField(DataType.Micro.name().toLowerCase());
			...
			e.printStackTrace();
			logger.error(String.format("处理微博数据异常：%s%n原始数据：%s%n", e.toString(), data));
		}
	}
	...
	private synchronized Map<Integer, Object> save(Map<String, Object> jsonMap) {
		Map<Integer, Object> result = GlobalObject.persistService.saveWeiBo(jsonMap);
		if (!result.containsKey(DataServiceConstants.success)) {
			logger.warn(String.format("微博持久化失败！错误编码：%s 原始数据：%s", result.keySet(), jsonMap));
			error.incrementAndGet();
		} else {
			success.incrementAndGet();
		}
		if (success.get() % 1000 == 0) {
			logger.info(String.format("Micro类型入库成功：%s 失败：%s", success.get(), error.get()));
		}
		return result;
	}
}
