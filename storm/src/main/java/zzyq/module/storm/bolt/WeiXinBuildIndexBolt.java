...
public class WeiXinBuildIndexBolt extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		try {
			boolean batch = false;
			if (tuple.contains(DataType.WeiXin.name().toLowerCase())) {
				Object object = (Object) tuple.getValueByField(DataType.WeiXin.name().toLowerCase());
				if (object instanceof String && ((String) object).equals(Config.batch)) {
					batch = true;
				} else {
					Weixin weixin = (Weixin) object;
					cachedMap.put(weixin.getId(), weixin);
				}
			}
			// 批量提交
			if (batch || (cachedMap.size() > 0 && cachedMap.size() >= Config.batchSize)) {
				...
				for (Entry<String, Object> entry : cachedMap.entrySet()) {
					...
					if (value instanceof Weixin) {
						IndexRequestBuilder requestBuilder = GlobalObject.elasticSearchClient.getClient()
								.prepareIndex(Config.es_indexName, DataType.WeiXin.name(), id).setSource(json);
						bulkRequestBuilder.add(requestBuilder);
						if(i==cachedMap.size()-1) {
							Weixin weixin = (Weixin)value;
							Index index = (Index) GlobalObject.crawlerMongo.find(Index.class, Weixin.class.getSimpleName().toLowerCase());
							if (null == index) {
								index = new Index(Weixin.class.getSimpleName().toLowerCase(), weixin.getTimestamp());
							}
							index.setTimestamp(weixin.getTimestamp());
				...
				if (numberOfActions > 0) {
					...
					if (!bulkResponse.hasFailures()) {
						...
						logger.info(String.format("微信索引构建成功！批量提交数量：%s 耗时：%s毫秒  总量：%s", numberOfActions, took, amount));
					}
					cachedMap.clear();
				}
			}
	...
	public void prepare(Map config, TopologyContext context, OutputCollector collector) {
		...
		GlobalObject.elasticSearchClient.mapping(Config.es_indexName, DataType.WeiXin.name());
	}
